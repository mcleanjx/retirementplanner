...
def _nav_buttons(step: int, can_next: bool = True, next_label: str = "Next →"):
    ...
    with cols[0]:
        if step > 1 and st.button("← Back", width='stretch'):
            st.session_state.wizard_step = step - 1
            st.rerun()
    with cols[2]:
        if st.button(next_label, disabled=not can_next, type="primary", width='stretch'):
            return True
    return False
...
- Investment approach: **{w['investment_style']}** ({_RETURN_BY_STYLE[w['investment_style']]*100:.0f}% return)
    ...
    with c1:
        if st.button("← Edit Answers", width='stretch'):
            st.session_state.wizard_step = 1
            st.session_state.wizard_complete = False
            st.rerun()
    with c2:
        if st.button("Show Results →", type="primary", width='stretch'):
            st.session_state.wizard_complete = True
            st.session_state.wizard_ever_completed = True
            st.rerun()
...
def _render_scenario_controls(w: dict, profile: dict, assumptions: dict,
    ...
    if c_save.button("💾 Save", key="simp_sc_save", width='stretch'):
        try:
    ...
    if saved:
        ...
        if c_load.button("Load", key="simp_sc_load", width='stretch'):
            try:
                ...
                st.error(str(e))
        if c_del.button("🗑", key="simp_sc_del", width='stretch',
...
def run_simplified_mode():
    ...
    with c_adv:
        if st.button("Switch to Advanced Mode →", width='stretch', type="primary"):
